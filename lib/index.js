...
const InlineStemMacroRx = /(?<!\\)stem:\[((?:[^\\\]]|\\.)*?)\]/gs
...
module.exports = function downdoc (asciidoc, { attributes: initialAttrs = {}, mathSyntax } = {}) {
  // Convert CRLF line endings to LF in memory
  asciidoc = asciidoc.replace(/\r\n/g, '\n')

// Extract stem macros before any other processing
  const passthroughContents = []
let processedAsciiDoc = asciidoc.replace(InlineStemMacroRx, (match, expr) => {
  // Only unescape \] → ], as this escape is required by AsciiDoc macro syntax.
  const cleaned = expr.replace(/\\\]/g, ']').trim()
  passthroughContents.push(cleaned);
  return `stem:[${passthroughContents.length - 1}]`
})

  const attrs = new Map(Object.entries(Object.assign({}, ATTRIBUTES, initialAttrs)))
  const lines = processedAsciiDoc.split(attrs.delete('doctitle') ? '\n' : '\n')
      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipStack.length && (match = skipStack[skipStack.length - 1] || (line === 'endif::[]' && line))) {
          ...
          if ((grab = DELIMS[line]) === 'v') {
            ...
            if (blockAttrs && blockAttrs.get(1) === 'stem') {
              if (mathSyntax === 'dollars') {
                line = '$$'
                inContainer.cap = '$$'
              } else {
                line = '```math'
