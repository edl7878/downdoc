...
downdoc automatically converts CRLF line endings (`\r\n`) to LF (`\n`) in memory, so documents with Windows-style line endings are supported.
...
* inline stem macro (supports multi-line content; always converted to dollar-delimited inline math)
...
You can control the syntax used for block math (stem blocks) using the `--math-syntax` option.
By default, stem blocks are converted to Markdown code blocks with the `math` language.
When `--math-syntax=dollars` is specified, stem blocks are converted to dollar-delimited display math blocks (`$$...$$`).

[,console]
----
$ npx downdoc --math-syntax=dollars README.adoc
----

This option only affects block stem macros (delimited with `++++`), not inline stem macros.
Inline stem macros are always converted to dollar-delimited inline math (`$...$`) and support multi-line content.
...
The following options are supported in the API:

* `attributes`:: An object (i.e., map) of runtime AsciiDoc attributes
* `mathSyntax`:: Specifies the math syntax for stem blocks; accepts `'dollars'` to convert stem blocks to dollar-delimited display math blocks (default: converts to Markdown code blocks with `math` language)
